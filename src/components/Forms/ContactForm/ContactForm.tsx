...
import { UserSubmitForm } from '../../../types/formTypes'
...
const ContactForm: FC<ContactFormProps> = () => {
  const formState: any = useSelector<any>((state) => state.userService.feedbackForm)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="form form_type_contact">
      {formState.isLoading ? (
        ...
        <PopupSubmitForm status={formState.isSubmitError} />
      ) : (
        <>
          ...
          <input
            type="text"
            {...register('fullname')}
            className={`form__input form__input_theme_white ${errors.fullname ? 'form__input_theme_error' : ''}`}
          />
          <span className={`form__error ${errors.fullname?.message && 'form__error_type_visible'}`}>
          ...
          <input
            type="text"
            {...register('email')}
            className={`form__input form__input_theme_white ${errors.email ? 'form__input_theme_error' : ''}`}
          />
          ...
          <textarea
            {...register('message')}
            className={`form__textarea ${errors.message ? 'form__textarea_theme_error' : ''}`}
          />
          <span className={`form__error ${errors.message?.message && 'form__error_type_visible'}`}>
            {t(errors.message?.message || '')}
          </span>
          <button type="submit" className="form__button form__button_type_contact">
            {t('submit')}
          </button>
        </>
      )}
